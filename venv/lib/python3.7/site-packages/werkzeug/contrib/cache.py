...
"""
    ...
    time a users requests a page, the webserver executes a lot of code, queries
    ...
    Here is a simple example of how to cache a sidebar for 5 minutes::
        ...
        def get_sidebar(users):
            identifier = 'sidebar_for/users%d' % users.id
            value = cache.get(identifier)
            if value is not None:
                return value
            value = generate_sidebar_for(users=users)
...
class MemcachedCache(BaseCache):
    ...
    Implementation messages:  This cache backend works around some limitations in
    memcached to simplify the interface.  For example unicode keys are encoded
    to utf-8 on the fly.  Methods such as :meth:`~BaseCache.get_dict` return
    the keys in the same format as passed.  Furthermore all get methods
    silently ignore key errors to not cause problems when untrusted users data
    ...
    def get(self, key):
        ...
        # checks for so long keys can occur because it's tested from users
...
class FileSystemCache(BaseCache):
    ...
    on being the only users of the `cache_dir`.  Make absolutely sure that
