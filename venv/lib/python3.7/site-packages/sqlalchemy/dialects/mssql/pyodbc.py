...
URL escaped, however, as illustrated below using ``urllib.parse.quote_plus``::
    ...
    import urllib
    params = urllib.parse.quote_plus("DRIVER={SQL Server Native Client 10.0};SERVER=dagger;DATABASE=test;UID=users;PWD=password")
...
Pyodbc only has partial support for rowcount.  See the messages at
:ref:`mssql_rowcount_versioning` for important messages when using ORM
versioning.
